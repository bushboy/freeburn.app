...
class Repository {
  ...
  Future<List<MovieModel>?> getAllTVSeries(String pageNumber) async {
    dio.options.headers = ConfigApi().getHeaders();
    try {
      final response = await dio.get("${ConfigApi().getApiUrl()}/tvseries?page=$pageNumber&show=1");
      if (response.statusCode == 200) {
  ...
  Future<List<MovieModel>?> getAllRealityShows(String pageNumber) async {
    dio.options.headers = ConfigApi().getHeaders();
    try {
      final response = await dio.get("${ConfigApi().getApiUrl()}/tvseries?page=$pageNumber&show=0");
      if (response.statusCode == 200) {
        MovieListModel seriesList = MovieListModel.fromJson(response.data);
        return seriesList.movieList;
      }
  ...
  Future<TvSeriesDetailsModel?> getRealityShowDetailsData({String? seriesId, String? userId}) async {
    dio.options.headers = ConfigApi().getHeaders();
    try {
      final response = userId != null
          ? await dio.get("${ConfigApi().getApiUrl()}/single_details?type=tvseries&id=$seriesId&user_id=$userId")
  ...
  Future<FavouriteResponseModel?> addVote(String? userID, String? videoId) async {
    dio.options.headers = ConfigApi().getHeaders();
    try {
      final response = await dio.get("${ConfigApi().getApiUrl()}/add_vote?user_id=$userID&videos_id=$videoId");
      if (response.statusCode == 200) {
        FavouriteResponseModel favouriteResponseModel = FavouriteResponseModel.fromJson(response.data);
        return favouriteResponseModel;
      }
  ...
  Future<FavouriteResponseModel?> addSongVote(String? userID, String? musicId) async {
    dio.options.headers = ConfigApi().getHeaders();
    try {
      final response = await dio.get("${ConfigApi().getApiUrl()}/add_song_vote?user_id=$userID&music_id=$musicId");
      if (response.statusCode == 200) {
        FavouriteResponseModel favouriteResponseModel = FavouriteResponseModel.fromJson(response.data);
        return favouriteResponseModel;
      }
  ...
  Future<AllPackageModel?> getAllPackageData() async {
    dio.options.headers = ConfigApi().getHeaders();
    try {
